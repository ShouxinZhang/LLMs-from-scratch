...
description: 当任务要求在 `.agents/sandbox/` 下隔离工作区时使用。适用于需要按“时间（精确到秒）+ task_name”创建任务目录、将脚本/中间产物/最终结果集中写入该目录、并尽量不污染仓库主目录的场景。
---
...
2. `mkdir -p` 创建 `.agents/sandbox/<timestamp>+<task_name>/`。
3. 所有新文件优先落到该目录。
4. 如有新增或修改文件，按 `workspace-docs` 要求更新文档说明。
